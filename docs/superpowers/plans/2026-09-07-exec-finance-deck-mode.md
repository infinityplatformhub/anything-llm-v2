...
| `server/utils/agents/aibitat/plugins/create-files/pptx/finance-layouts.js` | create | renderer 9 layout + `formatNumber(value, unit)` / `formatPct(n)` + `addDeckFooter(slide, theme, footer)` + `roundedAxisMax(max)` + `fixEmbeddedChartTables(buffer)`; ใช้ `slide.addChart` (ชื่อจริงตามที่ ship; plan เดิมเขียน `formatBaht`) |
...
- [x] `themes.js`: เพิ่ม `executive` (titleSlideBackground `0C1929`, accent `C9943E`, background `FFFFFF`, titleColor `0C1929`, bodyColor `2C3E50`, `chartColors: ["1A5276","C9943E","5A6D82","7B96B5","B8C4D0"]`, `chartPositive: "2E7D5B"`, `chartNegative: "B4392E"`, `chartNeutral: "5A6D82"`, `chartGrid: "ECE9E1"`, `statusGreen/Amber/Red`); theme อื่นทั้ง 5 ได้ chart tokens ค่า default เดียวกัน (`getTheme` merge) — ค่า negative/grid ตาม ruling "mockup tokens win" (plan เดิม B5483C/E3E8EE)
