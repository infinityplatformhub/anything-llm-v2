...
const LOGS = path.join(ROOT, "e2e/logs/mcp-servers-ui");
...
async function fakeProvider() {
  ...
  fake = http.createServer(async (req, res) => {
    try {
      ...
      if (["/mcp", "/plain-mcp"].includes(url.pathname)) {
        const plain = url.pathname === "/plain-mcp";
        const company = plain
          ? req.headers.authorization === `Bearer ${plainToken}` && "Workspace Company"
          : tokens.get((req.headers.authorization || "").replace(/^Bearer /, ""));
        if (!company) {
          if (!plain) res.setHeader("WWW-Authenticate", `Bearer resource_metadata="${origin}/.well-known/oauth-protected-resource"`);
        ...
        server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: [
          { name: "get_company", description: "Return the selected fake company", inputSchema: { type: "object", properties: {} } },
          ...(plain ? [{ name: "create_invoice", description: "Return a fake invoice without persisting data", inputSchema: { type: "object", properties: { invoice: { type: "object", description: "Invoice JSON" } }, required: ["invoice"] } }] : []),
        ] }));
        server.setRequestHandler(CallToolRequestSchema, async (request) => {
          if (request.params.name === "get_company") return { content: [{ type: "text", text: company }] };
          if (plain && request.params.name === "create_invoice") {
            const invoice = request.params.arguments?.invoice;
            if (!invoice || typeof invoice !== "object" || Array.isArray(invoice)) throw new Error("invoice must be an object");
            return { content: [{ type: "text", text: JSON.stringify({ status: "created", invoice }) }] };
          }
...
async function selfTest() {
  ...
  const plainCall = (token, method = "tools/list", params = {}) => fetch(`${origin}/plain-mcp`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", Accept: "application/json, text/event-stream" }, body: JSON.stringify({ ...rpc, method, params }) });
  const unauthorized = await plainCall("wrong-token");
  assert.equal(unauthorized.status, 401);
  assert.equal(unauthorized.headers.get("www-authenticate"), null);
  const plainTools = await plainCall(plainToken);
  assert.equal(plainTools.status, 200);
  assert.deepEqual((await plainTools.json()).result.tools.map((tool) => tool.name), ["get_company", "create_invoice"]);
  assert.ok((await (await plainCall(plainToken, "tools/call", { name: "create_invoice", arguments: {} })).json()).error);
  const invoice = await plainCall(plainToken, "tools/call", { name: "create_invoice", arguments: { invoice: { customer: "Harness" } } });
  assert.equal(JSON.parse((await invoice.json()).result.content[0].text).invoice.customer, "Harness");
  console.log("Plain MCP self-test passed: wrong bearer rejected without OAuth, both tools listed, required object validated, invoice echoed.");
}
...
async function main() {
  ...
  // Generate only into temp storage: shared node_modules can have an older schema.
  const clientDir = path.join(environment.root, "prisma-client");
  const schema = fs.readFileSync(environment.schemaPath, "utf8");
  const isolatedSchema = schema.replace(/(generator client\s*\{)/, `$1\n  output = ${JSON.stringify(clientDir)}`);
  if (isolatedSchema === schema) throw new Error("Cannot isolate Prisma generator output");
  fs.writeFileSync(environment.schemaPath, isolatedSchema);
  fs.symlinkSync(path.join(SERVER_DIR, "node_modules"), path.join(environment.root, "node_modules"), "dir");
  execFileSync(process.execPath, [path.join(SERVER_DIR, "node_modules/prisma/build/index.js"), "generate", "--schema", environment.schemaPath], {
    cwd: environment.root, env: { ...process.env, PRISMA_GENERATE_SKIP_AUTOINSTALL: "true" }, stdio: "pipe",
  });
  const preload = path.join(environment.root, "isolated-prisma.cjs");
  fs.writeFileSync(preload, `const Module = require("node:module");
const resolve = Module._resolveFilename;
Module._resolveFilename = function(request, ...args) {
  return request === "@prisma/client" ? ${JSON.stringify(path.join(clientDir, "index.js"))} : resolve.call(this, request, ...args);
};
...
`);
  ...
  fs.writeFileSync(authFile, JSON.stringify({ admin, viewer, plain: { url: `${origin}/plain-mcp`, token: plainToken } }), { mode: 0o600 });
  ...
  console.log(`Plain MCP: ${origin}/plain-mcp\nTest-only Authorization: Bearer ${plainToken}`);
  const report = path.join(ROOT, ".infi/e2e-report.json");
  fs.mkdirSync(path.dirname(report), { recursive: true });
  const playwright = spawn(process.execPath, [path.join(ROOT, "node_modules/@playwright/test/cli.js"), "test", "-c", "e2e/mcp-servers-ui.playwright.config.ts", ...process.argv.slice(2), "--headed", "--reporter=json"], { cwd: ROOT, env: { ...process.env, MCP_UI_AUTH_FILE: authFile, PLAYWRIGHT_JSON_OUTPUT_FILE: report }, stdio: "inherit" });
