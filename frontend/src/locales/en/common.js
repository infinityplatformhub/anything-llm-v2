const TRANSLATIONS = {
  ...
  agent: {
    ...
    mcp: {
      ...
      delete: "Delete",
      manage: "Manage",
      "manage-server": "Manage {{name}}",
      "edit-title": "Edit {{name}}",
      "workspace-owned": "Workspace-owned",
      "shared-globally": "Shared globally",
      "empty-title": "No MCP servers yet",
      "empty-description": "Add an HTTP / SSE server for this workspace or enable a server from the shared catalog. Local stdio servers must still be configured globally.",
      "shared-catalog": "Open shared catalog",
      "workspace-credentials": "Uses this workspace's configured headers. Enable to make its tools available to this workspace's agent.",
      "shared-credentials": "Uses this instance's shared credentials. Enabling grants this workspace access to its tools.",
      "read-only": "Read-only view. Only administrators can add, edit, test, delete, connect, disconnect, or enable MCP servers.",
      "modal-description": "Only for workspace {{workspace}}. Supports HTTP / SSE. Secrets are encrypted before storage.",
      "config-tabs": "Server configuration format",
      form: "Form",
      json: "JSON",
      name: "Server name",
      "name-hint": "Use 2–64 lowercase letters, numbers, underscores or hyphens. Start with a letter or number.",
      transport: "Transport",
      url: "URL",
      "url-hint": "Use an http(s) URL without embedded credentials. The server checks host safety for this deployment.",
      headers: "Headers",
      "add-header": "Add header",
      "header-key": "Header key {{index}}",
      "header-value": "Header value {{index}}",
      "remove-header": "Remove header {{index}}",
      "clear-header": "Clear header value {{index}}",
      clear: "Clear",
      "secret-hint": "Header names containing token, key, secret, password, authorization or cookie are masked on read. Leave a masked value unchanged to keep it, replace it to update it, or remove the row to delete it.",
      "oauth-required": "Requires OAuth per workspace",
      "oauth-hint": "After saving, use Connect on the card to sign in. An Authorization header is not needed for OAuth.",
      "allowed-tools": "Tools available to the agent",
      "extra-servers": "{{count}} additional servers from JSON will also be saved.",
      "json-hint": "Paste a vendor's mcpServers block or a single server definition. Unknown fields and local stdio commands are not supported.",
      "single-name-hint": "Used only for a single definition without an mcpServers wrapper.",
      "json-config": "MCP server JSON",
      "json-valid": "Valid JSON for {{count}} servers.",
      "use-form": "Use in form",
      "test-connection": "Test connection",
      "cancel-test": "Cancel test",
      "test-idle": "Not tested",
      "test-testing": "Connecting…",
      "test-ok": "Connected",
      "test-error": "Connection failed",
      "test-success": "Connected · {{count}} tools · {{latency}} ms",
      "test-error-details": "The server rejected this probe. Check the URL and authentication configuration. Credentials are not shown in errors.",
      "tool-list": "Tool list",
      details: "Details",
      "untested-warning": "This configuration has not passed a connection test. You can save it, but tool calls may fail until the configuration is corrected.",
      "test-first": "Test connection probes only the first server in this JSON block.",
      "created-names": "Created: {{names}}. Correct the remaining servers below and save again.",
      save: "Save",
      saving: "Saving…",
      saved: "MCP server configuration saved. Enable the server to use its tools.",
      cancel: "Cancel",
      "test-tools": "Test tools",
      "test-tools-title": "Test tools · {{name}}",
      "real-data-warning": "Tools call the real server using this workspace's credentials and may create, update, or delete real data. Review the tool and arguments before running it.",
      "enable-before-run": "Enable this server in the workspace before running tools.",
      "loading-tools": "Loading tools…",
      "choose-tool": "Choose tool",
      "choose-value": "Choose a value",
      "no-tools": "This server returned no tools.",
      "no-arguments": "This tool does not require arguments.",
      "argument-required": "{{name}} is required.",
      "argument-number": "{{name}} must be a valid number of the required type.",
      "argument-json": "{{name}} must contain valid JSON.",
      "argument-type": "{{name}} must be a JSON {{type}}.",
      run: "Run",
      running: "Running…",
      retry: "Try again",
      "raw-result": "Raw result",
      "result-latency": "Raw result · {{latency}} ms",
      copy: "Copy result",
      copied: "Result copied.",
      "copy-failed": "Unable to copy. Select and copy the result manually.",
      truncated: "The server truncated this result to its response size limit.",
      "delete-title": "Delete {{name}}?",
      "delete-warning": "This deletes the server configuration, OAuth tokens, and enable state from workspace {{workspace}}. Its running client will stop immediately. Other workspaces are not affected.",
      "confirm-delete": "Delete server",
      deleted: "MCP server and workspace connection deleted.",
      errors: {
        request_failed: "Unable to complete the MCP request. Please try again.",
        admin_required: "Only administrators can change or test MCP servers.",
        invalid_json: "Invalid JSON syntax. Check quotes, commas, and brackets.",
        invalid_body: "Provide a non-empty mcpServers object or a server name and configuration.",
        invalid_config: "The server configuration must be a JSON object.",
        invalid_name: "Server names must use 2–64 lowercase letters, numbers, underscores or hyphens, starting with a letter or number.",
        invalid_url: "Provide a valid http(s) URL without credentials. This host may not be allowed by the server.",
        invalid_type: "Transport must be SSE or Streamable HTTP.",
        invalid_headers: "Headers must have unique HTTP token names and string values without line breaks, control characters, or characters beyond Latin-1.",
        invalid_masked_header: "This masked header has no saved value. Enter its actual value or remove the header.",
        invalid_mcp_servers: "Provide a non-empty mcpServers object or exactly a server name and configuration.",
        invalid_probe_timeout: "The server's connection-test timeout is invalid. Ask an administrator to check the deployment configuration.",
        invalid_slug: "The workspace identifier is invalid. Reload this workspace and try again.",
        invalid_arguments: "Tool arguments must be a valid JSON object and include a tool name.",
        operation_failed: "The server could not complete this operation. Please try again.",
        create_failed: "This server could not be created. Correct its configuration and try again.",
        list_failed: "Unable to load the MCP server catalog. Please try again.",
        headers_too_large: "Headers exceed the server limit of 20 keys or 4 KB.",
        invalid_anythingllm: "The anythingllm field must be an object.",
        invalid_per_workspace_auth: "perWorkspaceAuth must be true or false.",
        invalid_suppressed_tools: "suppressedTools must be an array of tool names.",
        stdio_not_supported: "Workspace servers do not support stdio. Remove the command, args, or env field and configure local servers globally. Offending field: {{field}}",
        unknown_field: "The configuration contains an unsupported field. Remove it before saving. {{field}}",
        name_conflict: "A workspace or global server already uses this name. Choose another name.",
        not_found: "This server no longer exists in the workspace. Refresh and try again.",
        workspace_not_found: "This workspace could not be found.",
        server_not_found: "This server could not be found in the workspace.",
        probe_failed: "Unable to connect to this server. Check the URL, headers, and OAuth connection.",
        probe_timeout: "The connection test timed out. Check the server and try again.",
        call_failed: "The tool call failed. Check the arguments and the workspace's server connection.",
        not_connected: "This server is not connected. Connect or enable it before running tools.",
        not_enabled: "Enable this server in the workspace before running tools.",
        missing_token: "Connect this server with OAuth before testing or running tools.",
        busy: "A test or tool call is already running for this server. Wait for it to finish.",
        edit_single_server: "Editing supports only this server. Keep its name unchanged and provide one definition.",
        save_masked_first: "This draft includes unchanged masked secrets. Save your changes first, then use Test tools, or replace the masked values to test this draft.",
      },
    },
    settings: {
