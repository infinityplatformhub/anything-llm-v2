...
function createCoverPageSection(docx, options) {
  const { Paragraph, TextRun, AlignmentType, Footer, SectionType } =
    docx;

  const { title, subtitle, author, date, theme, margins } = options;
...
/**
 * Creates a running footer with Page X of Y.
 * @param {Object} docx - The docx library
 * @param {Object} theme - Theme color configuration
 * @returns {Object} Footer configuration
 */
function createRunningFooter(docx, theme) {
